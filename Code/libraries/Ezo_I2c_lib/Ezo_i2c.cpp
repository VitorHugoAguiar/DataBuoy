...
void Ezo_board::send_cmd_with_num(const char* cmd, float num, uint8_t decimal_amount, const char* unity){
	String temp = String(cmd )+ String(num, decimal_amount) + String(unity);
...
void Ezo_board::send_read_with_salinity_comp(float salinity){
	send_cmd_with_num("S,", salinity, 3, ",ppt");
	this->issued_read = true;
}
